...
/***/ function(module, exports, __webpack_require__) {
	...
	var YiZTable = __webpack_require__(159);
	...
	var FilterableTable = React.createClass({displayName: "FilterableTable",
	    ...
	    render: function() {
	        ...
	        var head2 = ["醫院", "社福中心", "辦公室電話", "主任", "主任手機", "附註"];
	...
	$.ajax({
	  ...
	  dataType: 'json'
	   ...
	   React.render(React.createElement(FilterableTable, {data: data.data, source: data.source, lastmodify: data.lastmodify, info: Info}), document.body);     
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rows = React.createClass({displayName: "Rows",
		render: function() {
			...
			var name = (text.length > 1) ? text.replaceAt(1, "○") : text;
			var match = false;

			for (var ind in this.props.data) {
				if ((ind == 3 && this.props.data[ind] != "主任") || ind == "檢傷編號" || ind == 12 || ind == "刪除註記")
					continue;


				var data = this.props.data[ind];
				data = data === null ? '' : data.toString();
				var key = ind + data;

				if (data.indexOf(text) > -1 && text) {
					match = true;
					rows.push(
					...
					rows.push(
						React.createElement("td", {key: key}, data)
					);
				}
			}
	...
	var YiZTable = React.createClass({displayName: "YiZTable",	
		componentWillMount: function() {
		},
		componentDidMount: function() {
		},
		render: function() {		
			...
			return (
				React.createElement("div", null, 
					React.createElement("table", {id: "YiZ-Table"}, 
						React.createElement("thead", null, 
							React.createElement(Rows, {data: this.props.head, filterText: ""})
						), 
